...
def plot_function(example_criterion, weights):
    x_grid = np.linspace(0, 20, 100)
    y_grid = [example_criterion(x, weights) for x in x_grid]
    fig, ax = plt.subplots()
...
def plot_history(evaluated_points, argmin, weights):
    """Plot the function and all evaluated points."""
    fig, ax = plot_function(example_criterion, weights)
    sns.rugplot(evaluated_points, ax=ax)
    ax.plot(
        [argmin],
        [example_criterion(argmin, weights)],
        marker="*",
        color="firebrick",
    )
    return fig, ax
